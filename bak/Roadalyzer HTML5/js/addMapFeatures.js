...
xChangeImg.onload = function () {
    alert("hello world");
};
//Add Signs.
function addSigns(stage, signsLayer, scale, totalPixels, windowWidth, messageLayer, signs) {
    for (var e = 0; e < signs.length; e++) {
        (function () {
            ...
            var yVar = 60;
            if (xVar < 8064) {
                addSign(stage, signsLayer, messageLayer, xVar, yVar, id);
            }
        }());
    }
    stage.draw();
...
// Add Interchanges
function addInterchanges(stage, interchangeLayer, scale, totalPixels, windowWidth, messageLayer, interchanges) {
    ...
    {(function () {
        ...
        var xVar = Math.round(((5280 * interChgArray[g].REFPT)/scale)*72);
        var yVar = 50;
        if (xVar < 8064) {
...
// Add Culverts
	  function addCulverts(stage,culvertLayer,scale,totalPixels,windowWidth, messageLayer,culverts) {
	   var culvertIdArray = culverts;
         //alert(culvertIdArray.length);
	   for (var i = 0; i < culvertIdArray.length; i++) {
	       //
	       var xLocation = Math.round(((5280 * culvertIdArray[i].MILEPOST) / scale) * 72);
	       if (xLocation < 8064) {
	           (function () {
...
// Adding Tick Marks to the two canvases.
	  ...
	  function addTickMarks(stage,stage2, ticksLayer, ticksLayer2,messageLayer, scalefactor, scaleUnits, wWidth) {
	      var xMark = 0;
	      var feetIndicator = 0;
	      ...
	      for (var j = 0; j < totalInchMarks + 1; j++) {
	          ...
	          (function () {
	              addTick(stage, stage2, ticksLayer, ticksLayer2, messageLayer, j, xMark, scalefactor, scaleUnits);
              }());
              xMark += 72;
	      }
	  ...
	  function addTick(stage, stage2, ticksLayer, ticksLayer2, messageLayer, newj, newx, scaleTicks, scaleUnits) {
	      ...
	      var subJ = Math.round(newj * scaleTicks) + (scaleTicks / 4);
	      for (var k = 0; k < 3; k++) {
	      ...
	      var xLoc = Math.round(newj * scaleTicks);
	  ...
	  function addSubTick(stage, stage2, ticksLayer, ticksLayer2, messageLayer, sx, scaleSubTicks, nJ,units) {
	      ...
	      //ticksLayer.add(subInchLine);
	      //ticksLayer2.add(subInchLine2);
	      //ticksLayer.draw();
	     // ticksLayer2.draw();
	      //stage.draw();
	      //stage2.draw();
...
// Add the actual bars here...
	  function addDataBars(stage2, dataBarLayer, scale, scaleUnits, windowWidth, messageLayer) {
	      var windowHt = 75;
	      var bar1Init = null;
	      var bar1x = 0;
          // Cost Center valuations:
	      for (var cst = 0; cst < costCenter.length; cst++) {
	          var barInitX = Math.round(((5280 * costCenter[cst].REFPT) / scale) * 72);
	          var barEndX = Math.round(((5280 * costCenter[cst].ENDREFPT) / scale) * 72);
	          ...
	          if (barInitX < windowWidth) {
	              addBar(stage2, dataBarLayer, scale, costBarColor(costCenter[cst].ELEM_VALUE), barValue1, barValue2, barValue3, barInitX, barEndX, barWidth, messageLayer, windowHt);
	          }
	      }
	      windowHt += 40;
          // Construction Year:
	      for (var cst = 0; cst < constYear.length; cst++) {
	          var barInitX = Math.round(((5280 * constYear[cst].REFPT) / scale) * 72);
	          var barEndX = Math.round(((5280 * constYear[cst].ENDREFPT) / scale) * 72);
	          var barWidth = barEndX - barInitX;
	          //var barColor = costBarColor(costCenter[cst].ELEM_VALUE);
	          ...
	          if (barInitX < windowWidth) {
	              addBar(stage2, dataBarLayer, scale, yearBarColor(constYear[cst].ELEM_VALUE), barValue1, barValue2, barValue3, barInitX, barEndX, barWidth, messageLayer, windowHt);
	          }
	      }
	      windowHt += 40;
          // Median Type:
	      for (var cst = 0; cst < medianType.length; cst++) {
	          var barInitX = Math.round(((5280 * medianType[cst].REFPT) / scale) * 72);
	          var barEndX = Math.round(((5280 * medianType[cst].ENDREFPT) / scale) * 72);
	          var barWidth = barEndX - barInitX;
	          //var barColor = costBarColor(costCenter[cst].ELEM_VALUE);
	          ...
	          if (barInitX < windowWidth) {
	              addBar(stage2, dataBarLayer, scale, medTypeBarColor(medianType[cst].ELEM_VALUE), barValue1, barValue2, barValue3, barInitX, barEndX, barWidth, messageLayer, windowHt);
	          }
	      }
	      windowHt += 40;
          // Median Width:
	      for (var cst = 0; cst < medianWidth.length; cst++) {
	          var barInitX = Math.round(((5280 * medianWidth[cst].REFPT) / scale) * 72);
	          var barEndX = Math.round(((5280 * medianWidth[cst].ENDREFPT) / scale) * 72);
	          var barWidth = barEndX - barInitX;
	          //var barColor = costBarColor(costCenter[cst].ELEM_VALUE);
	          ...
	          if (barInitX < windowWidth) {
	              addBar(stage2, dataBarLayer, scale, medWidthBarColor(medianWidth[cst].ELEM_VALUE), barValue1, barValue2, barValue3, barInitX, barEndX, barWidth, messageLayer, windowHt);
	          }
	      }
	      windowHt += 40;
          // Suffiency Ratings:
	      for (var cst = 0; cst < suffRate.length; cst++) {
	          var barInitX = Math.round(((5280 * suffRate[cst].REFPT) / scale) * 72);
	          var barEndX = Math.round(((5280 * suffRate[cst].ENDREFPT) / scale) * 72);
	          var barWidth = barEndX - barInitX;
	          //var barColor = costBarColor(costCenter[cst].ELEM_VALUE);
	          ...
	          if (barInitX < windowWidth) {
	              addBar(stage2, dataBarLayer, scale, suffRatingBarColor(suffRate[cst].ELEM_VALUE), barValue1, barValue2, barValue3, barInitX, barEndX, barWidth, messageLayer, windowHt);
	          }
	      }
	      windowHt += 40;
          // Surface Material:
	      for (var cst = 0; cst < surfMat.length; cst++) {
	          var barInitX = Math.round(((5280 * surfMat[cst].REFPT) / scale) * 72);
	          var barEndX = Math.round(((5280 * surfMat[cst].ENDREFPT) / scale) * 72);
	          var barWidth = barEndX - barInitX;
	          //var barColor = costBarColor(costCenter[cst].ELEM_VALUE);
	          ...
	          if (barInitX < windowWidth) {
	              addBar(stage2, dataBarLayer, scale, surfMatBarColor(surfMat[cst].ELEM_VALUE), barValue1, barValue2, barValue3, barInitX, barEndX, barWidth, messageLayer, windowHt);
	          }
	      }
	      windowHt += 40;
          // Number of Lanes:
	      for (var cst = 0; cst < numLanes.length; cst++) {
	          var barInitX = Math.round(((5280 * numLanes[cst].REFPT) / scale) * 72);
	          var barEndX = Math.round(((5280 * numLanes[cst].ENDREFPT) / scale) * 72);
	          var barWidth = barEndX - barInitX;
	          //var barColor = costBarColor(costCenter[cst].ELEM_VALUE);
	          ...
	          if (barInitX < windowWidth) {
	              addBar(stage2, dataBarLayer, scale, numLanesBarColor(numLanes[cst].ELEM_VALUE), barValue1, barValue2, barValue3, barInitX, barEndX, barWidth, messageLayer, windowHt);
	          }
	      }
	      windowHt += 40;
	  ...
	  function addBar(stage2, dataBarLayer, scale, barColor, barValue1,barValue2,barValue3, barInitX, barEndX, barWidth, messageLayer,y) {
	      ...
	      (function () {
	          ...
	          box.on("mousemove", function () {
	              ...
	              var offsets = stage2._getContainerPosition();
